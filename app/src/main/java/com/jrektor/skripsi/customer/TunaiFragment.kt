...
import kotlinx.android.synthetic.main.fragment_tunai.*

class TunaiFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_tunai, container, false)

        val progres = view.findViewById<ProgressBar>(R.id.pb_consumer)
    ...
    private fun getConsumer(outlet: String) {
        ...
        if (activity != null) {
            ...
            val request = JsonArrayRequest(
                Request.Method.GET,
                GlobalData.BASE_URL + "order/apiorder.php?in_outlet=$outlet",
                null,
                { response ->
                    if (response.length() == 0) {
                        ...
                        rv_consumer_cash.layoutManager = LinearLayoutManager(requireContext())
                        rv_consumer_cash.adapter = adapter
                    }
                },
                { error ->
